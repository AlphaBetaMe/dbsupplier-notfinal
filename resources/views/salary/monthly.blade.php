            ...
            <tbody>
                @foreach ($timeLogs as $timeLog)
                    <tr>
                        ...
                        <td> {{ \Carbon\Carbon::parse($timeLog->time_out)->diff(\Carbon\Carbon::parse($timeLog->time_in))->format('%h hours %i minutes')}}</td>
                        <td>Php{{ \Carbon\Carbon::parse($timeLog->time_out)->diffInMinutes(\Carbon\Carbon::parse($timeLog->time_in)) / 60 * 62.5}}</td>
                    </tr>
                    @php $total += \Carbon\Carbon::parse($timeLog->time_out)->diffInMinutes(\Carbon\Carbon::parse($timeLog->time_in)) / 60 * 62.5 ; @endphp
                @endforeach
